...
def play_request(request, song):
    items = PlaylistItem.objects.filter(song__code=song, playlist_id=REQUEST_PLAYLIST)
    items.delete();
    return view_chord(request, song)


def request_song(request, song):
    song = Song.objects.get(code=song)
    playlist = Playlist.objects.get(id=REQUEST_PLAYLIST)
...
def view_playlist(request, playlist):
    ...
    context['requesting'] = playlist.id == REQUEST_PLAYLIST
...
def request_view(request, ordering=None):
    context = get_context(request)

    items = PlaylistItem.objects.filter(playlist_id=REQUEST_PLAYLIST)
    items = list(items)
    ...
    if not items:
        return HttpResponse("NO SONG IN PLAYLIST.")

    if ordering > last_item:
        ordering = last_item
    elif ordering < 0:
        ordering = 0

    song = items[ordering].song
    context['code'] = song.code
    context['chord'] = find_chord_image(song.code)

    context['prev'] = ordering if ordering == 0 else ordering - 1
    context['next'] = ordering if ordering == last_item else ordering + 1
...
def search(request):
    ...
    for view in views:
        result.append({
            'code': view['song__code'],
            'description': view['song__description'],
            'view': view['count'],
